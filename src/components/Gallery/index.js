...
class Gallery extends Component {
  state = {
    activeImageId: 0,
    // finalList: imagesList[0],
  }

  filteredImagesList = () => {
    const {activeImageId} = this.state
    // console.log(activeImageId)
    const filteredList = imagesList.filter(each => each.id === activeImageId)
  ...
  render() {
    ...
    let final = this.filteredImagesList()
    final = {...final}
    console.log(final[0].imageUrl)
    return (
      <div className="bg-container">
        <div className="card-container">
          <img
            src={final[0].imageUrl}
            alt={final[0].imageAltText}
            className="image"
          />
        </div>
        <div>
